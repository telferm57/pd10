...
def recordEndWalk(walkCount,stepFirst,stepLast,stepCount,avact,epoch):
    # startwalk in epoch format 
    global walklog
    wst = timedelta(seconds=stepFirst)
    wst = pd.to_datetime(epoch[0]) + wst
    walklog[walkCount] = [stepFirst,stepLast,stepCount,avact,wst]
    return
...
def getWalks(y,actclasses,smawindow,epoch,samplerate=50):
    ...
    for i in np.arange(len(y)):
        ...
        # !! Fallen below threshold !!                                  
            ...
            if x[i]-step[stepCount-1] > walkGapMaxs: # max gap allowed in a walk 
                        ...
                        recordEndWalk(walkCount,step[0],step[stepCount-1],stepCount-1,avact,epoch)
                        walkState = 0 
                        stepCount = 0 
            else:
...
def getGaitfeatures(walklog,arb,walkno='all',samplerate=50): 
    ''' input : 
  ...
  #
        ...
        #  walk=46; twstart, twend, steps, act, dtime = walks[walk]
        if (steps > 5) & ((act > 0.5) & (act < 1.8)):  # walklength(no. steps)  > 2 
            print('start walk ',walk,steps,act)
            twstart *= samplerate
            twend *= samplerate
           # twstart = twstart-10  # walk identification starts from  centre of step
          #  twend = twend + 10 # gait function needs start of step + lead in/out
            twstart = int(twstart)
            twend = int(twend)
            axyz = arb[twstart:twend,:].copy()
         #   plt.plot(axyz)
        #   vacc18 = vacc[twstart:twend]
            axyz /= 9.81 # mhealthx expects values in g not ms-2
